...
class PostgresUtil():
    ...
    def __init__(self,
        ...
        self.__database_url = f"postgresql://{self.__username}:{self.__password}@{self.__host}:{self.__port}/{self.__database}"
    ...
    def dataframe_from_postgres(self, table_name):
        dataframe = pd.read_sql(f"SELECT * FROM {table_name}", self.__engine)
        return dataframe
